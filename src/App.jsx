  ...
  const renderCostInput = () => {
    return (
      <div>
        ...
        {/* ═══ 현금유입 탭 ═══ */}
        {costTab === "inflow" && (() => {
          const monthItems = (cashflowItems[currentMonth] || []).filter(it => it.flow_type === "inflow");
          const getStatus = (it) => {
            const exp = toNum(it.expected_amount);
            const act = toNum(it.actual_amount);
            if (act <= 0) return { label: "미입금", bg: "#fee2e2", color: C.error };
            if (exp > 0 && act < exp) return { label: "일부입금", bg: "#ffedd5", color: C.orange };
            return { label: "입금완료", bg: "#dcfce7", color: C.success };
          };
          const updateItem = (idx, field, value) => {
            const updated = [...monthItems];
            updated[idx] = { ...updated[idx], [field]: value };
            setCashflowItems(prev => ({ ...prev, [currentMonth]: [...(prev[currentMonth] || []).filter(it => it.flow_type !== "inflow"), ...updated] }));
            debounceSave(`cf_${updated[idx].id || idx}_${field}`, () => saveCashflowItem({ ...updated[idx], [field]: value }));
          };
          const addInflowItem = async () => {
            setSavingStatus("saving");
            const newItem = { month: currentMonth, flow_type: "inflow", cost_group: "fixed", account_label: "", vendor: "", expected_day: null, expected_amount: 0, actual_date: null, actual_amount: 0, memo: "", sort_order: monthItems.length };
          ...
          const removeInflowItem = async (idx) => {
          ...
          const overallStatus = sumActual <= 0 ? { label: "미입금", color: C.error } : sumActual < sumExpected ? { label: "일부입금", color: C.orange } : { label: "입금완료", color: C.success };
          ...
          return (
            <div style={{ ...pcardStyle, overflowX: "auto" }}>
              ...
              <div style={{ display: "flex", gap: 12, marginBottom: 16, flexWrap: "wrap" }}>
                {[
                  { label: "예상 합계", value: pFmtFull(sumExpected) + "원", color: C.navy },
                  { label: "실제 합계", value: pFmtFull(sumActual) + "원", color: sumActual >= sumExpected && sumExpected > 0 ? C.success : C.orange },
                  { label: "차액", value: (sumExpected - sumActual > 0 ? "-" : "+") + pFmtFull(Math.abs(sumExpected - sumActual)) + "원", color: sumExpected - sumActual > 0 ? C.error : C.success },
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11 }}>
                <thead>
                  <tr style={{ background: C.navy }}>
                    {["계정과목", "거래처", "예상일", "예상금액", "실제일", "실제금액", "상태", "메모", ""].map(h => (
                      <th key={h} style={{ padding: "8px 6px", color: "#fff", fontWeight: 700, textAlign: "center", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {monthItems.length === 0 && (
                    <tr><td colSpan={9} style={{ padding: 40, textAlign: "center", color: C.gray, fontSize: 13 }}>
                      항목이 없습니다. 아래 "+ 항목 추가" 버튼으로 현금유입 항목을 등록하세요.
                    </td></tr>
                  )}
                  {monthItems.map((it, i) => {
                      ...
                      <tr key={it.id || i} style={{ background: i % 2 === 0 ? "#fff" : C.bg, borderBottom: `1px solid ${C.border}` }}>
                        <td style={{ padding: "4px 6px", width: 120 }}>
                          <input value={it.account_label || ""} onChange={e => updateItem(i, "account_label", e.target.value)}
                            placeholder="계정과목" style={{ ...inputStyle, padding: "6px 8px", fontSize: 12, width: "100%" }} />
                        </td>
                        <td style={{ padding: "4px 6px", width: 120 }}>
                          <input value={it.vendor || ""} onChange={e => updateItem(i, "vendor", e.target.value)}
                            placeholder="거래처" style={{ ...inputStyle, padding: "6px 8px", fontSize: 12, width: "100%" }} />
                        </td>
                        <td style={{ padding: "4px 6px", width: 60 }}>
                          <input type="number" inputMode="numeric" value={it.expected_day ?? ""} min={1} max={31}
                            ...
                            placeholder="일" style={{ ...inputStyle, padding: "6px 4px", fontSize: 12, width: "100%", textAlign: "center" }} />
                        </td>
                        <td style={{ padding: "4px 6px", width: 130 }}>
                        ...
                        <td style={{ padding: "4px 6px", textAlign: "center", width: 70 }}>
                          <span style={{ display: "inline-block", padding: "3px 8px", borderRadius: 20, fontSize: 10, fontWeight: 800, background: st.bg, color: st.color }}>{st.label}</span>
                        </td>
                        <td style={{ padding: "4px 6px", width: 120 }}>
                          <input value={it.memo || ""} onChange={e => updateItem(i, "memo", e.target.value)}
                            placeholder="메모" style={{ ...inputStyle, padding: "6px 8px", fontSize: 12, width: "100%" }} />
                        </td>
                        <td style={{ padding: "4px 6px", textAlign: "center", width: 30 }}>
                          <button onClick={() => removeInflowItem(i)} style={{ background: "none", border: "none", cursor: "pointer", color: C.error, fontSize: 14 }}>✕</button>
                        </td>
                      </tr>
                    );
                  })}
                  ...
                  {monthItems.length > 0 && (
                    <tr style={{ background: C.navy }}>
                      <td colSpan={3} style={{ padding: "8px 6px", color: C.gold, fontWeight: 900, textAlign: "center" }}>합계 ({monthItems.length}건)</td>
...
function DailyReportPage({ employees, onDataChange, initialDate, initialSite }) {
  ...
  const startNew = (siteCode) => {
    ...
    const siteEmps = employees.filter(e => (e.site_code_1 === site || e.site_code_2 === site) && e.status === "재직");
    setForm({
      site_code: site,
  ...
  const openPanel = (siteCode) => {
    ...
    if (report) {
      ...
      if (existing) { setEditMode(false); }
        ...
        const siteEmps = employees.filter(e => (e.site_code_1 === siteCode || e.site_code_2 === siteCode) && e.status === "재직");
        setForm({
          site_code: siteCode,
          valet_count: 0, valet_amount: 0, memo: "",
          staffList: siteEmps.map(e => ({ employee_id: e.id, name_raw: e.name, staff_type: "regular", work_hours: 8 })),
      ...
      <div style={{ ...cardStyle, borderLeft: `4px solid ${C.gold}` }}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          {isCrewRole ? (
            ...
            <select value={form.site_code} onChange={e => { const code = e.target.value; setForm(f => ({ ...f, site_code: code, staffList: employees.filter(emp => (emp.site_code_1 === code || emp.site_code_2 === code) && emp.status === "재직").map(emp => ({ employee_id: emp.id, name_raw: emp.name, staff_type: "regular", work_hours: 8 })) })); }} style={fieldSt} disabled={!!form.id}>
              {FIELD_SITES.map(s => <option key={s.code} value={s.code}>{s.code} {s.name}</option>)}
            </select>
          )}
        </div>
...
const PAGE_LABELS = {
  dashboard: "HR 대시보드", employees: "직원현황",
...
function MainApp() {
  ...
  // 수익성 분석 공유 state (ProfitabilityPage)
...
const ATT_STATUSES = [
  ...
  { key: "지원", label: "지원", color: "#FFE0B2", text: "#E97132", bg: "#FFF3E0" },
...
function calcPersonalAttStats(empId, workCode, dates, getCellStatusFn, todayStr) {
  ...
  return { att, extra, support, late, absent, leave, offDay, worked, regularWorked, totalWorkable, attRate, lateRate };
}
              ...
              <div style={{ background: "#fff", border: "1.5px solid #E8ECF4", borderRadius: 14, overflow: "hidden" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: actualPay !== null ? "1fr 1fr 1fr 1fr" : "1fr 1fr 1fr", gap: 0 }}>
                  {[
                    ...
                    { label: "예상 급여", value: fmt(payCalc.expectedPay), sub: (empStats.extra > 0 || empStats.support > 0) ? `출근${empStats.regularWorked}${empStats.extra > 0 ? "+추가" + empStats.extra : ""}${empStats.support > 0 ? "+지원" + empStats.support : ""}/${empStats.totalWorkable}일` : `출근 ${empStats.regularWorked}/${empStats.totalWorkable}일`, color: payCalc.diff < 0 ? C.error : C.success },
            ...
            <div style={{ background: "#fff", border: "1.5px solid #E8ECF4", borderRadius: 14, padding: "14px 18px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 8 }}>
                {[
                  ...
                  { label: "지원", value: empStats.support || 0, color: "#E97132" },
...
function SiteAnalyticsTab({ employees, year, month, dates, getCellStatus, todayStr, staffRows, reports, extraAmountMap, moveMonth, goToday, loading }) {
  ...
  const exportSiteExcel = async () => {
    ...
    const rows1 = siteStats.map(s => ({
      ...
      총결근: s.totalAbsent, 총지각: s.totalLate, 지원: (s.emps || []).reduce((ss, d) => ss + (d.st.support || 0), 0), 추가근무: s.totalExtra, 추가수당합계: s.totalExtraAmt,
    }));
    ...
    siteStats.forEach(s => s.emps.forEach(d => {
      rows2.push({
        ...
        출근: d.st.att, 추가: d.st.extra, 지원: d.st.support || 0, 지각: d.st.late, 결근: d.st.absent, 연차: d.st.leave,
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: FONT }}>
              <thead>
                <tr style={{ background: "#F8F9FC" }}>
                  {["#", "사업장", "인원", "출근률", "출근", "지원", "결근", "지각", "추가근무", "추가수당"].map(h => (
              ...
              <tbody>
                {siteStats.map((s, i) => (
                  <tr key={s.code} onClick={() => setSiteFilter(s.code)} style={{ cursor: "pointer", borderBottom: "1px solid #F0F2F8", background: i % 2 === 0 ? "#fff" : "#FAFBFD" }}>
                    ...
                    <td style={{ padding: "10px 8px", textAlign: "center", color: (s.emps || []).reduce((ss, d) => ss + (d.st.support || 0), 0) > 0 ? "#E97132" : C.gray, fontWeight: 700 }}>{(s.emps || []).reduce((ss, d) => ss + (d.st.support || 0), 0)}</td>
        ...
        <div style={{ background: "#fff", border: "1.5px solid #E8ECF4", borderRadius: "0 0 14px 14px", overflow: "hidden", marginBottom: 16 }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12, fontFamily: FONT }}>
              <thead>
                <tr style={{ background: "#F8F9FC" }}>
                  {["#", "사번", "이름", "근무형태", "출근률", "출근", "추가", "지원", "지각", "결근", "연차", "등급", "예상급여", "추가수당"].map(h => (
              ...
              <tbody>
                {selectedSite.emps.map((d, i) => (
                  <tr key={d.emp.id} style={{ borderBottom: "1px solid #F0F2F8", background: i % 2 === 0 ? "#fff" : "#FAFBFD" }}>
                    ...
                    <td style={{ padding: "9px 6px", textAlign: "center", color: (d.st.support || 0) > 0 ? "#E97132" : C.gray, fontWeight: 700 }}>{d.st.support || 0}</td>
              ...
              <tfoot>
                <tr style={{ background: "#F0F4FF", borderTop: `2px solid ${C.navy}` }}>
                  ...
                  <td style={{ padding: "10px 6px", textAlign: "center", fontWeight: 800, color: "#E97132" }}>{selectedSite.emps.reduce((ss, d) => ss + (d.st.support || 0), 0)}</td>
...
function AttendancePage({ employees }) {
  ...
  const { autoAttMap, supportDetailMap } = useMemo(() => {
    const map = {};
    const sdMap = {}; // supportDetailMap: { "empId-date": { siteCode, siteName, checkIn, checkOut, memo } }
    const repDateMap = {};
    const repSiteMap = {}; // report_id → site_code
    reports.forEach(r => {
      repDateMap[r.id] = r.report_date;
      repSiteMap[r.id] = r.site_code;
    });

    // 직원 id → workCode / site_code_1 빠른 조회
    const empWorkCodeMap = {};
    const empSiteMap = {};
    employees.forEach(e => {
      empWorkCodeMap[e.id] = e.work_code;
      empSiteMap[e.id] = e.site_code_1;
    });
    ...
    staffRows.forEach(s => {
      ...
      // (지원/타사업장은 offDay여도 정상 처리)
      if (!isPeak && !isExtra && !isSupport && !isCrossSite) {
      ...
      // 지원 판정
      if (isSupport || isCrossSite) {
        // supportDetailMap에 상세 기록
        const siteName = SITES.find(st => st.code === reportSite)?.name || reportSite || "";
        sdMap[key] = {
          siteCode: reportSite || "",
          siteName,
          checkIn: s.check_in || "",
      ...
      if (!map[key]) {
        ...
        if (map[key] !== "출근") map[key] = "출근";
      } else if (isPeak && (map[key] === "추가" || map[key] === "지원")) {
        map[key] = "피크";
      }
    });

    // daily_report_extra: 직원 지정된 추가항목 → 해당 날짜 마킹
    // (이미 출근/피크/지원으로 잡힌 날은 덮어쓰지 않음)
    extraRows.forEach(e => {
    ...
    return { autoAttMap: map, supportDetailMap: sdMap };
  ...
  const getCellStatus = (empId, dateStr, workCode) => {
    ...
    if (autoAttMap[key]) return autoAttMap[key]; // "출근" / "추가" / "피크" / "지원"
  ...
  const kpiSupportCount = allFilteredEmps.reduce((sum, emp) => {
    return sum + dates.filter(d => getCellStatus(emp.id, d.dateStr, emp.work_code) === "지원").length;
  }, 0);
  ...
  const overallAttRate = expectedTotal > 0 ? Math.round(((kpiAttCount + kpiExtraCount + kpiPeakCount + kpiSupportCount + kpiLateCount) / expectedTotal) * 100) : 0;
  ...
  const empStats = useMemo(() => {
    return allFilteredEmps.map(emp => {
      ...
      const support = dates.filter(d => getCellStatus(emp.id, d.dateStr, emp.work_code) === "지원").length;
      ...
      const extraAmt = extraAmountMap[emp.id] || 0;
      return { ...emp, att, extra, support, late, absent, leave, worked, rate, extraAmt };
    });
  ...
  const exportExcel = async () => {
    ...
    const summaryRows = empStats.map(e => ({
      ...
      "근무형태": getWorkLabel(e.work_code), "출근": e.att, "추가": e.extra, "지원": e.support || 0,
      "지각": e.late, "결근": e.absent, "연차": e.leave, "근무일수": e.worked,
      "출근률(%)": e.rate, "추가수당(원)": e.extraAmt,
    }));
    const ws1 = XLSX.utils.json_to_sheet(summaryRows);
    ws1["!cols"] = [{ wch: 12 }, { wch: 8 }, { wch: 14 }, { wch: 12 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 8 }, { wch: 10 }, { wch: 14 }];
    ...
    const siteRows = siteGroups.map(g => {
      ...
      const gSupport = gEmps.reduce((s, e) => s + (e.support || 0), 0);
      ...
      const gRate = gExpected > 0 ? Math.round(((gAtt + gExtra + gSupport + gLate) / gExpected) * 100) : 0;
      return { "사업장": `${g.code} ${g.name}`, "인원": gEmps.length, "출근": gAtt, "추가": gExtra, "지원": gSupport, "지각": gLate, "결근": gAbsent, "출근률(%)": gRate, "추가수당합계(원)": gExtraAmt };
    });
    const ws3 = XLSX.utils.json_to_sheet(siteRows);
    ws3["!cols"] = [{ wch: 18 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 6 }, { wch: 10 }, { wch: 16 }];
    ...
    <div style={{ padding: "24px 28px", maxWidth: 1400, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(100px, 1fr))", gap: 10, marginBottom: 20 }}>
        {[
          ...
          { icon: "🔄", label: "지원", value: `${kpiSupportCount}건`, color: "#E97132" },
      ...
      {/* P5 근무유형별 횟수 → 캘린더 테이블 우측에 인라인 통합 (v9.4) */}
      ...
      <div style={{ background: "#fff", border: "1.5px solid #E8ECF4", borderRadius: 14, overflow: "hidden" }}>
        <div style={{ overflowX: "auto", overflowY: "auto", maxHeight: "calc(100vh - 200px)" }}>
          <table style={{ borderCollapse: "collapse", width: "100%" }}>
            ...
            <tbody>
              {siteGroups.length === 0 && (
                <tr><td colSpan={daysInMonth + 8} style={{ padding: 40, textAlign: "center", color: C.gray, fontSize: 16 }}>
              ...
              {siteGroups.map(group => (
                <Fragment key={group.code}>
                  {/* 사업장 그룹 헤더 */}
                  <tr>
                    <td colSpan={daysInMonth + 8} style={{ padding: "2px 6px", background: "#EEF1F8", fontSize: 13, fontWeight: 800, color: C.navy, borderBottom: "1px solid #E8ECF4" }}>
                      🏢 {group.name} <span style={{ color: C.gray, fontWeight: 600 }}>({group.code}) · {group.emps.length}명</span>
                      <span style={{ marginLeft: 6, fontSize: 11, fontWeight: 600 }}>
                        ...
                        {group.emps.filter(e => getWorkCat(e.work_code) === "parttime").length > 0 && <span style={{ color: "#2E7D32", marginLeft: 3 }}>알바{group.emps.filter(e => getWorkCat(e.work_code) === "parttime").length}</span>}
                      </span>
                    </td>
                  </tr>
                  {group.emps.map((emp, idx) => {
                    // 유형별 근무횟수 계산
                    let wd = 0, we = 0, ex = 0, pk = 0, sp = 0, kd = 0, hol = 0;
                    dates.forEach(d => {
                    ...
                    const totalWorked = wd + we + ex + pk + sp + hol;
                    const dutyVals = [
                      ...
                      <tr key={emp.id} style={{ background: idx % 2 === 0 ? "#fff" : "#FAFBFC" }}>
                        ...
                        {(() => {
                          ...
                          return dates.map(d => {
                            ...
                            const isWorked = st === "출근" || st === "추가" || st === "피크" || st === "지원" || st === "지각";
                            ...
                            return (
                              <td key={d.day}
                                ...
                                title={`${d.dateStr} (${d.dayName})${d.holidayName ? ` · ${d.holidayName}` : ""}${st ? ` — ${st}` : ""}${st === "지원" && supportDetailMap[`${emp.id}-${d.dateStr}`] ? ` → ${supportDetailMap[`${emp.id}-${d.dateStr}`].siteName}` : ""}${hasExtraPay ? ` 💰${fmt(extraAmountByDateMap[`${emp.id}-${d.dateStr}`])}원` : ""}`}
                              >
                                {cellText !== null && (
                                  <div style={{
                                    fontSize: 13, fontWeight: 700, color: info?.text || C.dark, padding: "2px 0", lineHeight: 1,
                                    ...(isAuto ? { borderBottom: `2px solid ${st === "추가" ? "#7C3AED" : st === "피크" ? "#D81B60" : st === "지원" ? "#E97132" : C.success}` } : {}),
                                  }}>
                                    {cellText}
                                  </div>
                                )}
                        ...
                        {dutyVals.map((dv, ci) => (
                          <td key={ci} style={{
                            ...
                            background: ci === 7 ? "#F8F9FC" : "transparent",
                          }}>
                            {dv.v || "·"}
                          </td>
                        ))}
                  ...
                  {/* 사업장 소계행 */}
                  {group.emps.length > 0 && (() => {
                    let sWd = 0, sWe = 0, sEx = 0, sPk = 0, sSp = 0, sKd = 0, sHol = 0;
                    ...
                    const sTotal = sWd + sWe + sEx + sPk + sSp + sHol;
                    const subVals = [
        ...
        {allFilteredEmps.length > 0 && (
          <div style={{ padding: "10px 16px", background: "#F8F9FC", borderTop: "1.5px solid #EEF1F8", fontSize: 14, color: C.gray, display: "flex", gap: 16, flexWrap: "wrap" }}>
            ...
            {kpiSupportCount > 0 && <span style={{ color: "#E97132", fontWeight: 700 }}>🔄 지원 {kpiSupportCount}건</span>}
      ...
      {popup && (
        <div onClick={e => e.stopPropagation()} style={{
          ...
          {/* 지원 상세정보 */}
          {getCellStatus(popup.empId, popup.dateStr, popup.workCode) === "지원" && supportDetailMap[`${popup.empId}-${popup.dateStr}`] && (() => {
            const sd = supportDetailMap[`${popup.empId}-${popup.dateStr}`];
            return (
              <div style={{ padding: "6px 10px", background: "#FFF3E0", borderRadius: 6, margin: "4px 0", fontSize: 12 }}>
                <div style={{ fontWeight: 800, color: "#E97132", marginBottom: 2 }}>🔄 지원 사업장</div>
                <div style={{ color: C.dark }}>{sd.siteName} ({sd.siteCode})</div>
                {(sd.checkIn || sd.checkOut) && <div style={{ color: C.gray, fontSize: 11 }}>{sd.checkIn || "?"} ~ {sd.checkOut || "?"}</div>}
                {sd.memo && <div style={{ color: C.gray, fontSize: 11, marginTop: 1 }}>{sd.memo}</div>}
              </div>
            );
          })()}
          {/* 삭제(초기화) */}
