...
| **소스코드** | `App.jsx` (~16,947줄) |
| **최신 커밋** | `412b93d` (2026.03.30) |
...
├─────────┴──────────────────────────────────────────┤
│ ⚠️ 계약 만료 D-7 알림 배너 (v10.1 F1)               │
│ ⚠️ 수습 만료 임박 배너 (v10.1 F2)                   │
│ P&L 테이블 (10열 + 합계 + 평균)                     │
├────────────────────────────────────────────────────┤
│ ⚠️ 월주차 만기 D-7 알림                             │
...
### 계약 만료 알림 배너 (v10.1 F1)
- D-7 이내 + 만료초과 계약 자동 감지 (`getContractExpiryInfo` 기반)
- 카드형 경고: 5단계 색상 + 이름 + 사업장 + 만기일
- HR 대시보드 바로가기 링크

### 수습 만료 알림 배너 (v10.1 F2)
- 수습만료 ±7일 이내 직원 자동 감지 (`probation_months` + `hire_date` 기준)
- 카드형 경고: D-day(빨강/주황) + 이름 + 사업장 + 종료일
- HR 대시보드 바로가기 링크 + 전환 처리 링크
