...
description: (주)미스터팍 미팍ERP_SYSTEM 통합 스킬 (v11). 4개 섹션 20개 화면: [HR&계약관리] HR대시보드(12KPI+계약KPI4+Recharts차트4종)/직원현황(계정관리통합+수습뱃지+전환워크플로우+퇴직관리탭)/계약이력/조항변경, [수익성분석★super_admin전용] 분석대시보드(가용자금KPI+현금흐름차트통합)/사업장PL/비용입력(6탭:매출+계약+간접비+현금유입+현금유출+카드)/급여대장(은행이체+급여내역서)/월주차관리/발렛비관리/비교분석/배부설정, [사업장현황] 사업장관리/현장일보/마감보고현황/근태현황/전체캘린더, [견적계산기★super_admin전용] 인건비견적. "계약서", "근로계약서", "직원현황", "사번관리", "사번중복", "동명이인", "contract", "임금테이블", "수습", "수습뱃지", "수습만료", "수습전환", "정규전환", "수습연장", "계약이력", "계약만료", "계약만료알림", "계약갱신", "퇴직관리", "퇴직금", "퇴사사유", "수익성", "사업장 분석", "PL", "손익", "간접비 배부", "매출 분석", "대시보드", "사업장 관리", "월주차", "주차", "견적서", "발렛 견적", "인건비", "현금흐름", "현금유입", "현금유출", "카드", "급여대장", "은행이체", "급여내역서", "payslip", "현장일보", "일보", "마감보고", "근태", "캘린더", "출결", "전화번호", "로그인", "연락처", "선택발송", "근태분석", "이상감지", "사업장삭제", "사업장명변경", "피크근무", "지원근무", "타사업장", "발렛비관리", "발렛비캘린더", "입퇴사추이", "근무유형", "간트차트", "사업장인건비", "권한분리" 등을 언급하면 이 스킬을 사용한다.
---

# 미팍ERP_SYSTEM 통합 스킬 (v11)

## 1. 개요

(주)미스터팍의 HR + 수익성분석 + 사업장현황 + 현금흐름 + 견적을 통합한 React(JSX) 기반 ERP 웹앱.
4개 메뉴 섹션, **총 20개 화면** (v11: 메인대시보드 삭제 → 분석대시보드에 통합).
...
| **소스코드** | `App.jsx` (~16,392줄) |
| **최신 커밋** | `76b0d9b` (2026.03.30) |
...
● 사업장 현황
  ...
  📆 전체 캘린더
─────────────────
● 수익성 분석               ★ super_admin 전용 (사이드바 비노출)
  📊 분석대시보드            ← 가용자금KPI + 현금흐름차트 통합
  🏢 사업장 PL
  ...
  ⚙️ 배부 설정
─────────────────
● 견적 계산기               ★ super_admin 전용 (사이드바 비노출)
  📋 인건비 견적
─────────────────
...
> ⚠️ v11 변경: 메인 대시보드 삭제(630줄), 기본 페이지 → HR대시보드(`dashboard`)
> ⚠️ v11 변경: 수익성분석/견적계산기 섹션 `isSuperAdmin` 전용 (admin은 사이드바에서 비노출)

### 역할별 메뉴 가시성 (4역할) — v11
| 역할 | 접근 가능 |
|------|----------|
| `super_admin` | 전체 화면 (수익성분석 + 견적 포함) |
| `admin` | HR & 계약관리 + 사업장현황만 (수익성분석/견적 비노출) |
...
## 6. 분석대시보드 (SummaryView) — v11 통합

> ⚠️ v11에서 메인 대시보드(MainDashboard) 630줄 삭제. 가용자금 KPI + 현금흐름 차트를 **분석대시보드(SummaryView)**에 통합.

### 레이아웃 구조
```
┌─ 월 선택 ─────────────────────────────────────────┐
├─ KPI 6칸 그리드 ──────────────────────────────────┤
│ 총매출 │ 총인건비 │ 간접비배부 │ 영업이익 │ 인건비율 │ 가용자금 │
├────────────────────────────────────────────────────┤
│ 수익 구조 바 (인건비/간접비/이익 비율)              │
├────────────────────────────────────────────────────┤
│ 사업장별 손익(P&L) 테이블 (정렬: 이익/이익률/매출/인건비) │
├────────────────────────────────────────────────────┤
│ 📊 현금흐름 추이 차트 (ComposedChart)               │
│   유입Bar(navy) + 유출Bar(error) + 잔액Line(gold)  │
└────────────────────────────────────────────────────┘
```

### 가용자금 KPI 계산 (v11 — SummaryView 내)
```javascript
...
### 현금흐름 추이 차트 (v11)
- Recharts `ComposedChart`: 전월 cashflowItems 키 기준 월별 자동 표시
- Bar: 유입(navy) + 유출(error, opacity 0.75), barSize 28
- Line: 잔액(gold), strokeWidth 2.5, dot r=4
- 이중 Y축: 좌=금액, 우=잔액 (억/만 단위 포맷)
- 커스텀 Tooltip (fullMonth + 항목별 금액)
...
- **계약현황 KPI 4개** (v10.1 F1): 계약만료임박(D-30) / 계약만료초과 / 금월갱신 / 갱신예정(D-90)
- **근로계약 만료 알림** (contractWarning): D-30 이내 직원 카드 (5단계 색상: D-90 스카이블루 / D-30 주황 / D-7 빨강 / D-day 진빨강 / 만료초과 짙은빨강)
- **사업장별 인원표**: 코드별 평일/주말/복합/알바/계 테이블 + 합계행
- **퇴사 현황**: 퇴사사유 분포 (resign_reason별 카운트)
- **Recharts 차트 4종** (v10.1 P6+):
  - `BarChart` 입퇴사 추이 (최근 12개월, 입사=navy / 퇴사=error)
  - `PieChart` 근무유형 도넛 (평일/주말/복합/알바, innerRadius=50 outerRadius=80)
  - `BarChart (layout=vertical)` 계약만료 타임라인 간트 (향후 90일, D-day 수평바, 4단계 색상, 최대 15명)
  - `BarChart` 사업장별 인건비 (만원 단위, 인원수 툴팁, 합계 서머리)
...
- **테이블 10열**: 사번 | 이름 | 직위 | 사업장 | 근무형태 | 기본급 | 일당 | 계정 | 상태 | 액션
  - v10.1: 이름셀에 **수습 뱃지** (`수습중 D-X` 주황 / `수습만료 D-X` 빨강 / `수습종료` 주황) + **계약만료 뱃지** (5단계 색상)
  - v10.1: 수습뱃지 클릭 → ProbTransitionModal(전환 워크플로우) 오픈
...
### 계약만료 알림 시스템 — `getContractExpiryInfo(emp, allContracts)` (v10.1 F1)
```javascript
// contracts 테이블에서 emp_no 매칭 + status='확정'/'갱신' + end_date 존재 → D-day 계산
// d < -30: 표시 안 함
// d < 0: "계약만료" (짙은빨강 #B71C1C, tier=0)
// d === 0: "오늘 만료!" (빨강, tier=1)
// d <= 7: "만료 D-X" (빨강, tier=2)
// d <= 30: "30일 이내" (주황, tier=3)
// d <= 90: "갱신예정" (스카이블루, tier=4)
```
- **5단계 알림**: tier 0~4, 메인대시보드 + HR대시보드 + 직원현황 3곳 연동
- **allContracts 상태관리**: MainApp에서 contracts 전체 로드 → Dashboard/EmployeeRoster에 prop 전달
...
### 수습→정규 전환 워크플로우 (v10.1 F2)
- **ProbTransitionModal**: 수습 만료 시 관리자 처리 모달 (3액션)
  1. **정규직 전환**: probation_months → null, 기본급 조정 옵션 (NumInput)
  2. **수습 연장**: 연장기간 선택 (1~6개월), hire_date 재산정 또는 probation_months 증가
  3. **계약 종료**: status → "퇴사", resign_date/resign_reason 자동 설정
- **진입점 4곳**: 수습뱃지 클릭 / 기본정보탭 전환버튼 / HR대시보드 전환링크 / 메인대시보드 전환링크
- **DB 변경**: employees 테이블 probation_months + base_salary + status + resign_* 업데이트
...
### 급여대장 (PayrollPage) — v11.1 급여연동 고도화
3탭 구성:
...
#### 직원현황 → 급여대장 자동 동기화 (v11.1)
- **SSOT 원칙**: 급여 기준은 직원현황(employees) 1곳에서만 관리
- **동기화 트리거 2곳**:
  1. `saveEmployee()`: 직원 저장 시 draft payroll_records 즉시 업데이트
  2. `loadPayrollMonth()`: 급여대장 페이지 진입 시 draft 레코드 재확인
- **동기화 대상 필드**: basic_pay(월급여), meal(식대), childcare(보육), car_allow(자가운전), team_allow(직책), incentive(인센티브), site_code, work_type, tax_type
- **보호 규칙**: confirmed 급여대장은 동기화 대상 제외 (과거 이력 보호)
- **식대 기본값 없음**: 직원현황 미입력 시 0 (기존 200,000 기본값 제거)

#### 급여대장 편집패널 (슬라이드 오버)
- **직원현황 급여**: 읽기전용 1줄 요약 (월급여/식대/보육 등 내역 표시, "수정은 직원현황에서" 안내)
- **수기수당**: NumInput 편집 가능 (급여대장 전용)
- **추가수당**: 일보 자동반영(auto) + 수동 추가(manual)
- **총 지급액**: 자동 계산
- **공제 내역**: 세금처리방식(4대보험/3.3%/3.3%타인/고용&산재/미신고) + 공제 수동입력
- **실입금액**: 총지급 - 공제

#### PY_PAY_FIELDS (급여대장 급여 구성)
```javascript
const PY_PAY_FIELDS = [
  { key: "basic_pay", label: "월급여" },  // ← "기본급" 아님 (임금테이블 분해값과 혼동 방지)
  { key: "meal", label: "식대" },
...
```
> ⚠️ `basic_pay`는 포괄임금 총액 (예: 6,000,000)이며, 직원현황의 `wd_basic`(임금분해 기본급 4,712,794)과 다름
...
- 공휴일 2026년 전면 수정 (v11.1): 설날(2/16-18), 부처님오신날(5/24), 추석(9/24-26), 대체공휴일 4개(3/2,5/25,8/17,10/5), 제헌절(7/17), 지방선거(6/3)
- `isHolidayForWorker(dateStr, workCode)`: 주말 공휴일은 주말근무자에게 일반주말, 평일 대체공휴일은 평일근무자에게 공휴 카운트
...
### v10.1 — HR 고도화 ✅ (현재)
HR 대시보드 전면 개편 + 수습뱃지 + 전환워크플로우 + 퇴직관리 + 사번 강화 + 계약만료 알림 + Recharts 차트 4종 + 메인대시보드 알림.
...
| **P8** | `0a5bff4` | F1 계약만료 5단계 알림 — getContractExpiryInfo 헬퍼, HR대시보드 계약KPI 4개+D-30 알림섹션, 직원현황 계약만료뱃지, allContracts 전역 상태 |
| **P9** | `30577ed` | F2 수습→정규 전환 워크플로우 — ProbTransitionModal(정규전환/수습연장/계약종료 3액션), 진입점4곳(뱃지클릭/기본정보탭/HR대시보드/메인대시보드) |
| **P10** | `412b93d` | F5 차트 보강 — 계약만료 타임라인 간트(D-day 수평바, 90일 이내, 4단계 색상) + 사업장별 인건비 BarChart(만원 단위, 합계 표시) |

### v11 — 구조 개편 + 권한 분리 ✅
메인대시보드 삭제, 분석대시보드에 가용자금KPI+현금흐름차트 통합, 수익성/견적 super_admin 전용.

| Phase | 커밋 | 내용 |
|-------|------|------|
| **P1** | `5453944` | MainDashboard 삭제 — 630줄 제거, main_dashboard 라우팅 삭제, PAGE_NAMES 정리 |
| **P2** | `9c8c4ae` | 권한 분리 — isSuperAdmin 변수, profitNavItems/calcNavItems super_admin 전용, 기본페이지 dashboard, 사이드바 수익성/견적 섹션 isSuperAdmin 래핑 |
| **P3** | `76b0d9b` | SummaryView 보강 — KPI 6칸(총매출/총인건비/간접비/영업이익/인건비율/가용자금) + 현금흐름 추이 ComposedChart(유입Bar+유출Bar+잔액Line) |

### v11.1 — 급여연동 + 공휴일 + 사직서 ✅ (현재)
직원현황→급여대장 SSOT 연동, 2026 공휴일 전면 수정, 사직서 서약사항 9개 조항, 퇴사자 근태 표시.

| Phase | 커밋 | 내용 |
|-------|------|------|
| **P1** | `e569301` | DB 마이그레이션 — employees 10+3컬럼 ALTER |
| **P2** | `2f3d3d7` | 명칭 변경 13곳 |
| **P3** | `dec25de` | 탭03 급여조건 재설계 — calcWdBreakdown/calcWeBreakdown, 읽기뷰+편집폼+saveEmp 자동산출 |
| **사직서** | `aec86f2`+`71baaff` | 사직서 서약사항 9개 조항 (인수인계/비밀유지/개인정보/자산반납/사고보고/지급일동의/손해배상 + 경업금지/차량사고 선택) |
| **근태** | `7d5393a` | 퇴사자 근태현황 표시 — 해당 월 일보 기록 있는 퇴사자 포함 + 퇴사 뱃지 |
| **공휴일** | `937f34e` | 2026 공휴일 전면 수정 — 설날/추석/부처님오신날 날짜, 대체공휴일 4개, 제헌절, 지방선거 + isHolidayForWorker |
| **급여연동** | `4f1bdc7`~`a51981b` | 직원현황→급여대장 SSOT 연동 — saveEmployee/loadPayrollMonth 자동동기화, 식대기본값 제거, 편집패널 간소화(급여필드 읽기전용) |
...
| 파일 | 용도 |
|------|------|
| `mrpark-contract-skill-v10.md` | 현재 스킬 (v11) |
